...
class HistoryManager {
  final searches = _SearchHistory();
  final songs = _SongHistory();
}

class _SearchHistory {
  final Box _box = Hive.box('SEARCH_HISTORY');
  ...
  List<Map<String, dynamic>> getList({String? filter}) {
    final searchHistory = filter == null
        ? _box.values
        : _box.values
            .where((el) => el.toLowerCase().contains(filter.toLowerCase()));
...
class _SongHistory {
  final Box _box = Hive.box('SONG_HISTORY');
